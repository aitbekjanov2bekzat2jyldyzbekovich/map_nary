  ...
  template: `
    <ul class="space-y-3">
      <li class="max-w-sm mx-auto">
        ...
        >
          <option v-for="i in select" :value="i.value">{{ i.name }}</option>
        </select>
      </li>
      ...
      <li v-show="$route.name === 'home'"> 
        ...
        <div class="relative">
          ...
          <input
            v-model="searchQuery"
            @input="$emit('searchMarkers', searchQuery)"
            type="search"
            id="default-search"
        ...
        >
          ...
          <div>
            <span class="font-medium">Предупреждение:</span>
            <p class="mt-1.5 list-disc list-inside">
              Поиск работает только по началу названия и чувствителен к регистру.
              Пожалуйста, вводите первые буквы правильно.
            </p>
          </div>
        </div>
      </li>
